...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_flat=torch.flatten(x_test,start_dim=1)
    for i in range(num_train):
      dists[i]=torch.sqrt(torch.sum((x_train_flat[i]-x_test_flat)**2,dim=1))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_train_flat=torch.flatten(x_train,start_dim=1).cuda()
    x_test_flat=torch.flatten(x_test,start_dim=1).cuda()
    x_train_pow=torch.sum(x_train_flat**2,dim=1,keepdim=True).cuda()
    x_test_pow=torch.sum(x_test_flat**2,dim=1).cuda()
    inner_prod=torch.mm(x_train_flat,x_test_flat.t()).cuda()
    dists=torch.sqrt(x_train_pow+x_test_pow-2*inner_prod).cpu()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for j in range(num_test):
      ...
      Dic={}
      for index in num_l:
        freq=y_train[indices[j]].tolist().count(index)
        Dic[index]=Dic.get(index,0)+freq
      sorted_Dic=sorted(Dic.items(),key=operator.itemgetter(1),reverse=True)
      Max_freq=sorted_Dic[0][1]
      keys=[key for key,value in Dic.items() if value==Max_freq]
      y_pred[j]=torch.tensor(min(keys))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for num in range(num_folds):
        x_train_pick=tuple(x_train_folds[i] for i in range(num_folds) if i != num)
        x_train_num=torch.cat(x_train_pick)
        x_test_num=x_train_folds[num]
        y_train_pick=tuple(y_train_folds[i] for i in range(num_folds) if i != num)
        y_train_num=torch.cat(y_train_pick)
        y_test_num=y_train_folds[num]
        knn_num=KnnClassifier(x_train_num,y_train_num)
        k_to_accuracies[k].append(KnnClassifier.check_accuracy(knn_num,x_test_num,y_test_num,k,quiet=True))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k,accs in sorted(k_to_accuracies.items()):
      if statistics.mean(accs)>statistics.mean(k_to_accuracies[best_k]):
        best_k=k
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
