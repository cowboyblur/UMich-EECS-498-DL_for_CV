...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    if begin==None:
      ...
      x=torch.arange(begin,stop+1,10).to(torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    z=torch.mm(x[0],y[0]).reshape(1,x.shape[1],y.shape[2])
    for i in range(1,x.shape[0]):
      z0=torch.mm(x[i],y[i]).reshape(1,x.shape[1],y.shape[2])
      z=torch.cat((z,z0),dim=0)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return z
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y=x.clone()
    x_col_mean=(torch.sum(x,dim=0))/x.shape[0]
    y=y-x_col_mean
    print(x_col_mean)
    x_col_std=torch.sqrt((torch.sum((x-x_col_mean)**2,dim=0))/(x.shape[0]-1))
    y=y/x_col_std
    print(x_col_std)
    ##########################################################################
    #                            END OF YOUR CODE                            #
